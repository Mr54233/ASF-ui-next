...
1. **BotCopyDialog 组件** ✅
   - 显示源 Bot 名称
   - 输入目标 Bot 名称
   - 表单验证（名称不能为空、不能为 ASF、不能重复）
   - 调用 `POST /api/Bot/{source}/Copy` API

2. **集成到 BotDetailDialog** ✅
   - 在快捷操作按钮区域添加"复制"按钮
   - 复制成功后刷新 Bot 列表

3. **集成到 Bots.vue** ✅
   - 添加 BotCopyDialog 组件
   - 添加 handleCopySuccess 回调
   - 在 handleDetailAction 中处理 copy 操作
...
**文件结构**:
```
src/components/BotCopyDialog.vue     # 复制弹窗组件
src/components/BotDetailDialog.vue   # 详情弹窗（添加复制按钮）
src/views/Bots.vue                   # Bot 列表页面（集成复制弹窗）
tests/components/BotCopyDialog.test.ts  # 4 个单元测试
```
...
**文件结构**:
```
src/components/BotDetailDialog.vue  # 详情弹窗组件（增强）
src/views/Bots.vue                   # Bot 列表页面
tests/views/Bots.test.ts             # 7 个单元测试
```
...
**解决方案**: 将点击事件分别放到卡片头部和内容区域，底部按钮区域不绑定点击事件
